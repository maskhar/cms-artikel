...
async function requireGlobalAdmin() {
  ...
  if (!user) return null;
  const { data: role } = await createAdminClient().schema("artikel").from("user_roles").select("id").eq("user_id", user.id).eq("role", "admin").is("site_id", null).maybeSingle();
...
export async function GET() {
  ...
  const [{ data: roles, error }, { data: users }] = await Promise.all([
    admin.schema("artikel").from("user_roles").select("id, user_id, site_id, role, sites(name, domain)").order("role"),
...
export async function POST(request: Request) {
  ...
  const { data, error } = await admin.schema("artikel").from("user_roles").insert({ user_id: target.id, site_id: input.siteId, role: input.role }).select("id").single();
