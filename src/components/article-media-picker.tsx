...
export function ArticleMediaPicker({ articleId, siteId, siteSlug, articleSlug, label, value, onChange }: ArticleMediaPickerProps) {
  ...
  async function uploadFile(file: File | undefined) { if (!file) return; if (!siteId) { setMessage("Pilih website sebelum upload gambar."); return; } if (!imageTypes.has(file.type) || file.size > 5 * 1024 * 1024) { setMessage("Pakai JPG, PNG, WebP, atau GIF maksimum 5 MB."); return; } const extension = file.name.split(".").pop()?.toLowerCase() || "jpg"; const path = `${siteSlug}/articles/${articleSlug}/${crypto.randomUUID()}.${extension}`; setUploading(true); setMessage(""); const storage = createClient().storage.from("artikel-media"); const { error } = await storage.upload(path, file, { contentType: file.type, upsert: false }); setUploading(false); if (error) { setMessage(error.message); return; } onChange(path); }
